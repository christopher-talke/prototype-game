/**
 * Entity palette body. Mirrors objectPalette for entityDefs.
 *
...
export interface EntityPaletteOptions {
    state: EditorWorkingState;
    recents: PaletteRecents;
    onPick: (defId: string) => void;
}

/** Build (or replace) the entity palette body in `container`. Returns refresh fn. */
export function mountEntityPalette(container: HTMLElement, opts: EntityPaletteOptions): () => void {
    container.innerHTML = '';
    container.classList.add('editor-palette', 'editor-palette-entity');
    ...
    const refresh = (): void => {
        ...
        const recentIds = getRecents(opts.recents, 'entity');
        const recentDefs = recentIds
            .map((id) => opts.state.map.entityDefs.find((d) => d.id === id))
        ...
        for (const def of opts.state.map.entityDefs) {
            if (filter && !defMatches(def.id, def.label, filter)) continue;
