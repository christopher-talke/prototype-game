...
- **Grid Displacement System**: `gridDisplacement.ts` - spring-damped physics on background grid points. Grid dots/lines displaced by player movement (wake effect) and explosions (radial shockwave via `GRENADE_DETONATE` events). 4 parallel `Float32Array`s for displacement/velocity, viewport-culled `Graphics` redraw each frame. Public API: `addDisplacementSource({x, y, radius, strength, duration, direction?})` / `removeDisplacementSource(id)` for external systems. `sceneGraph.ts` split into `backgroundRect` (static fill) + `gridGraphics` (dynamic per-frame). `SPRING_K=120`, `DAMPING=12` (slightly underdamped), `MAX_DISPLACEMENT=20`. Wired into `renderPipeline.ts`, `main.ts`, `rendererSwitch.ts`, `match.ts`.
