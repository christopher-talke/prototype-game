...
- **Phase 4 - Fog of War**: `computeRaycastPolygon()` now returns `{ vertices: coordinates[]; count: number } | null` (removed CSS string building from simulation layer). `raycastRenderer.ts` builds CSS polygon from vertices for DOM path. `pixiFogOfWar.ts`: Graphics stencil mask on `fogOfWarLayer` - a large rect with the visibility polygon `cut()` out, updated each frame in screen-space coords. FOV cone fallback as two Graphics lines in `fovConeLayer`. `renderPipeline.ts` dispatches raycast result to both renderers.
